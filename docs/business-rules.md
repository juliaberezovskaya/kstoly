...
- **BR-L1.8. (BR-ShoppingList.List.8)** Организатор и Гости по ссылке могут отмечать товары в списке покупок как купленные или не купленные.
...
- **BR-C0.2.** *Гость* — пользователь, перешедший по ссылке приглашения и не являющийся владельцем мероприятия.
...
- **BR-C4.1.** При открытии мероприятия система сравнивает `user_id` (если он существует) с `owner_id` мероприятия:
  ...
  - если не совпадают или `user_id` отсутствует → пользователь считается Гостем.
...
могут несколько пользователей (Организатор и Гости по ссылке).
