...
2. Система показывает ссылку-приглашение.
...
1. На шаге 4 основного потока система пытается скопировать ссылку в буфер обмена, но операция отклонена политиками браузера/ОС (например, нет разрешения, нет безопасного контекста, нет жеста пользователя).
2. Система отображает сообщение: «Не удалось автоматически скопировать ссылку».
3. Система отображает ссылку в виде текстового поля/модального окна так, чтобы Организатор мог скопировать её вручную (выделить и скопировать через системное меню/горячие клавиши).
4. *Если платформа поддерживает системную функцию “Поделиться”*, система также предлагает действие «Поделиться» (например, открыть системное меню отправки).
5. Организатор копирует ссылку вручную или отправляет её через системное меню. Сценарий завершается.
...
- Связанные FR:
- [FR-C0](../srs/3.%20Функции%20системы/3.4.%20Совместная%20работа.md#fr-c0%20управление-ссылкой-приглашением)
